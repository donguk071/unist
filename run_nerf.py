...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    return rays_o ,rays_d , ret_list + [ret_dict]

def depth_to_point(rgb, depth, rays_o, rays_d):
        
    rays_o_np = rays_o.detach().cpu().numpy().reshape(int(np.sqrt(rays_o.shape[0])),int(np.sqrt(rays_o.shape[0])),3)
    rays_d_np = rays_d.detach().cpu().numpy().reshape(int(np.sqrt(rays_o.shape[0])),int(np.sqrt(rays_o.shape[0])),3)
    
    rgb_cpu = rgb.detach().cpu().numpy()
    depth_cpu = depth.detach().cpu().numpy()
    
    # print(rays_d_np.shape)
    # print(rays_o_np.shape)
...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rays_o ,rays_d ,[rgb, disp, acc, depth, extras] = render(H, W, focal, chunk=chunk, c2w=c2w[:3,:4], retraw=True, **render_kwargs)
        ...
        #depth to point 부분 추가        
        if i % 3 == 0 :
            new_pointcloud = depth_to_point(rgb, depth, rays_o, rays_d)
            new_pointcloud_tuple = tuple(map(tuple, new_pointcloud))
            if not any(np.array_equal(new_pointcloud_tuple, pc_tuple) for pc_tuple in map(tuple, pointcloud)):
                pointcloud.append(new_pointcloud)
        ...
        if savedir is not None:
            ...
            depth = (depth / np.nanmax(depth) * 255).astype(np.uint8)
    ...
    txt_filename = os.path.join(savedir, 'pointcloud')
    np.save(txt_filename, pointcloud)
...
def config_parser():
    ...
    # optimizing 
    parser.add_argument("--smoothing_lambda", type=float, default=1, #1 
...
def train():
    ...
    ########################################
        ...
        i_train, i_test, i_val = i_split, i_split, i_split
        near = 2.
        far = 6.
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, disps = render_path(render_poses, hwf, args.chunk, render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir, render_factor=args.render_factor)
    ...
    for i in trange(start, N_iters):
        ...
        rays_o ,rays_d, [rgb, disp, acc, depth, extras]= render(H, W, focal, chunk=args.chunk, rays=batch_rays,
        ...
        render_kwargs_train['network_fn'].iteration += 1
        # print(render_kwargs_train['network_fn'].iteration)
